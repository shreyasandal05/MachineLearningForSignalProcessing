...
from scipy.signal import welch #for welch method


#generate a simple chirp signal
fs=1000 #Sampling frequency
t = np.linspace(0, 10, fs*10) # Time vector
signal = chirp(t, f0=6, f1=1, t1=10, method ='quadratic') # Frequnecy swept cosine generator
...
fs = len(t) / (t[-1] - t[0])  # Sampling frequency calculation
frequencies, psd = welch(signal, fs, nperseg=256)

# frequencies: Array of sample frequencies.
# psd: Power spectral density of the signal.plt.figure(figsize=(10, 5))
plt.semilogy(frequencies, psd)  # semilogy for log scale on y-axis
